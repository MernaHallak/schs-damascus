...
export default function ArticlesFiltersBar({
  ...
  const submitSearch = () => {
    const nextUrl = buildUrl(q, status);
    const currentQs = sp.toString();
    const currentUrl = currentQs ? `/admin/articles?${currentQs}` : "/admin/articles";
    ...
    >
      <div className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            <button
              ...
              aria-label="بحث"
              onClick={submitSearch}
              className="absolute left-2 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-xl border border-emerald-100 bg-emerald-50 text-emerald-700 shadow-sm transition hover:bg-emerald-100 active:scale-[0.98] cursor-pointer"
            >
              <Search className="h-4.5 w-4.5" strokeWidth={2.4} />
            </button>
          </div>
        ...
        <button
          type="button"
          onClick={handleReset}
          className="h-12 rounded-2xl border border-neutral-200 bg-white px-5 text-sm font-bold text-slate-800 shadow-sm transition hover:bg-neutral-50 cursor-pointer"
        >
          إعادة ضبط
        </button>
      </div>
